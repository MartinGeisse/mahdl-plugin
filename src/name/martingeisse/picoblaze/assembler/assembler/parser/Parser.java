...
public class Parser {
	...
	public void parse(final IParserClient client) throws IOException {
		while (true) {
			...
			} catch (final LineSyntaxException e) {
	...
	private void handlePragma(final IParserClient client, final Token commentToken, final String trimmedComment) {
		int i;
		for (i = 1; i < trimmedComment.length(); i++) {
			final char c = trimmedComment.charAt(i);
			if (c >= '0' && c <= '9') {
				continue;
			}
			if (c >= 'a' && c <= 'z') {
		...
		final String identifier = trimmedComment.substring(1, i);
		if (identifier.length() == 0) {
			errorHandler.handleWarning(commentToken, "pragma comment with empty identifier");
			return;
		}
		final String parameter = trimmedComment.substring(i).trim();
	 ...
	 */
	public void expected(final Token token, final String description) throws LineSyntaxException {
